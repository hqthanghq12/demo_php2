...
class DispatcherTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testReverseRouteWithDashes()
    {
        ...
        $r->any( array('student-catalogue/store/{store:i}?', 'products'), array(__NAMESPACE__.'\\Test','route'));

        $this->assertEquals('student-catalogue/store', $r->route('products'));
        $this->assertEquals('student-catalogue/store/1', $r->route('products', array(1)));
    }
    ...
    public function testPrefixGroupsReverseRoutes()
    {
        ...
        $r->group(['prefix' => 'student-catalogue/store'], function($r) {
            $r->any(['items/{store:i}?', 'products'], array(__NAMESPACE__.'\\Test','route'));
        });

        $this->assertEquals('student-catalogue/store/items', $r->route('products'));
        $this->assertEquals('student-catalogue/store/items/1', $r->route('products', array(1)));
    }
